...
pub struct SymbolEntry {
    result: Result<SymbolId, ErrorId>,
    visibility: Visibility,
}

impl SymbolEntry {
    pub fn result(&self) -> Result<SymbolId, ErrorId> { self.result }
...
impl NamespaceMap {
    ...
    pub fn get_double(&mut self, ns1: NamespaceId, ns2: NamespaceId) -> (&mut Namespace, &mut Namespace) {
        assert_ne!(ns1, ns2);
        let [ns1, ns2] = self.map.as_mut_slice()
            .get_disjoint_mut([ns1.usize(), ns2.usize()])
            .unwrap();
        (ns1, ns2)
    }
    ...
    pub fn get_sym(&self, owner: NamespaceId, requester: NamespaceId, name: StringIndex) -> SymbolGetResult {
        let Some(entry) = self.get_ns(owner).entry(name)
        else { return SymbolGetResult::Undefined };

        if entry.visibility() != Visibility::Public 
        && !self.can_access(owner, requester) {
            return SymbolGetResult::Private
        }

        match entry.result() {
            Ok(sym) => SymbolGetResult::Symbol(sym),
            Err(err) => SymbolGetResult::Errored(err),
        }
    }
    ...
    pub fn can_access(&self, owner: NamespaceId, mut requester: NamespaceId) -> bool {
        loop {
            ...
            let Some(parent) = self.get_ns(requester).parent 
            else { return false; };
            requester = parent;
        }
    }
...
impl Namespace {
    ...
    pub fn set_err_sym(&mut self, name: StringIndex, err: ErrorId) {
    ...
    pub fn get_sym(&self, name: StringIndex) -> Option<Result<SymbolId, ErrorId>> { self.entry(name).map(SymbolEntry::result) }
