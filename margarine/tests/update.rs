...
#[test]
fn install_manages_the_current_binary_and_target_toolchains_atomically() {
    let home = tempfile::tempdir().unwrap();
    let installation_root = home.path().join(".margarine");
    // Requests: release metadata + host toolchain checksum + host toolchain
    ...
    let output =
        Command::new(env!("CARGO_BIN_EXE_margarine"))
            ...
            .env("HOME", home.path())
            .env("MARGARINE_RELEASES_API", api)
            .env_remove("MARGARINE_TOOLCHAIN_DIR")
            .output()
            .unwrap();
    server.join().unwrap();
    ...
    assert!(!fs::read_dir(&installation_root).unwrap().any(|entry| {
        entry.unwrap().file_name().to_string_lossy().starts_with(".staging-")
    }));

    let second =
        Command::new(installation_root.join("bin/margarine"))
            .args(["install", "--yes"])
            .env("HOME", home.path())
    ...
    assert!(
        String::from_utf8_lossy(&second.stdout).contains("is already managed"),
        "stdout:\n{}",
