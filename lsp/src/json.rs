...
impl<'a> Parser<'a> {
    ...
    fn parse_string(&mut self) -> Result<&'a str, Error> {
        ...
        loop {
            let (slice, no_eoi) = self.reader.consume_while_slice(|&c| c != b'"' && c != b'\\');
...
impl From<f64> for Value<'static> {
    #[inline(always)]
    fn from(val: f64) -> Self { Value::Number(val) }
}

impl<'a> From<&'a str> for Value<'a> {
    #[inline(always)]
    fn from(val: &'a str) -> Self { Value::String(val) }
}

impl<'a> From<&'a [Value<'a>]> for Value<'a> {
    #[inline(always)]
    fn from(val: &'a [Value<'a>]) -> Self { Value::Array(val) }
}

impl<'a> From<&'a [(&'a str, Value<'a>)]> for Value<'a> {
    #[inline(always)]
    fn from(val: &'a [(&'a str, Value<'a>)]) -> Self { Value::Object(val) }
}
